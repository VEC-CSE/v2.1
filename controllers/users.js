                  ...
                  return res.render("index",{msg:"Password does not match :(  , If you have forgotten click Forgot password ,if none works contact site admin Mr Lavan J V :)"});
                   ...
                   let userid=result[0].userid;
                    db.query(
                      ...
                      { name: name, email: email,videolectures: videolectures,nptel: nptel,slideshare: slideshare,ndl: ndl,delnet: delnet,spokentutorial: spokentutorial,blogspot: blogspot,logintime: logintime,userid: userid,ejournal: ejournal,ebooks: ebooks,eshoudhsindhu: eshoudhsindhu,generalbookreference:generalbookreference,newspaperandarticle: newspaperandarticle,userid: userid},
                      (error, result) => {
                        if (error) {
                          console.log(error);
                        } 
...
const { name, email, password ,cpassword,userid} = req.body;
// console.log(name);
// console.log(email);
...
db.query("select email from users where email=?",[email],async (error,result) =>{
        ...
        db.query(
            ...
            { name: name, email: email, pass: hashedPassword ,userid: userid},
            (error, result) => {
              if (error) {
                console.log(error);
              } else {
...
exports.logout = async (req, res) => {
  ...
  db.query(
    ...
    (err, result) => {
      ...
      let userid=lname.userid;
      let logoutdate=new Date();

    db.query(
                      "insert into logout set ?",
                      { name: name, email: email,logoutdate: logoutdate,userid: userid},
                      (error, result) => {
                        if (error) {
                          console.log(error);
                        } 
